...
const questions: Question[] = [
	{
		...
		options: [
			...
			{
				emoji: "🚀",
				label: "Sistema integrado con stock en tiempo real",
				score: 4,
			},
		],
	},
	...
	},
	{
		...
		options: [
			...
			{
				emoji: "🔄",
				label: "Lo veo en un sistema, pero se desactualiza",
				score: 3,
			},
			{
				emoji: "✅",
				label: "Sí, en tiempo real desde cualquier dispositivo",
				score: 4,
			},
		],
	},
	{
		...
		options: [
			...
			{
				emoji: "🔀",
				label: "Otro puede cubrir, pero con limitaciones",
				score: 3,
			},
			{
				emoji: "💪",
				label: "Los procesos siguen sin depender de nadie",
				score: 4,
			},
		],
	},
	{
		...
		options: [
			...
			{
				emoji: "⚡",
				label: "CRM con alertas y seguimiento automático",
				score: 4,
			},
		],
	},
	{
		...
		options: [
			...
			{
				emoji: "🤖",
				label: "Se genera automáticamente desde el pedido",
				score: 4,
			},
		],
	},
	{
		...
		options: [
			...
			{
				emoji: "🎯",
				label: "Dashboards en tiempo real con datos integrados",
				score: 4,
			},
		],
	},
...
export default function DigitalDiagnostic() {
	...
	const [answers, setAnswers] = useState<
		Record<
			number,
			{ score: number; questionText: string; responseLabel: string }
		>
	>({});
			...
			try {
				recaptchaToken = await new Promise<string>((resolve, reject) => {
					...
					window.grecaptcha.ready(() => {
						window.grecaptcha
							.execute(RECAPTCHA_SITE_KEY, { action: "diagnostic_lead" })
							.then(resolve)
							.catch(reject);
					});
				});
			} catch {
				// If reCAPTCHA fails, still proceed but without token (server will reject if strict)
								...
								{question.options.map((option, index) => {
									...
									return (
										<motion.button
											...
											type="button"
											onClick={() =>
												onSelectOption(question.id, option.score, index)
											}
			...
			>
				...
				<div className={`rounded-2xl border p-5 sm:p-8 ${archetype.tone}`}>
					<div className="flex items-center gap-3 sm:block">
						...
						<h2 className="text-2xl sm:mt-3 sm:text-3xl font-bold text-white">
							{archetype.name}
						</h2>
					</div>
					<p className="mt-2 text-base sm:mt-3 sm:text-xl font-medium text-white/90">
						{archetype.tagline}
					</p>
				...
				<div className="mt-3 sm:mt-6 rounded-xl border border-white/10 bg-navy-surface p-4 sm:p-6 flex items-center gap-4 sm:block sm:text-center">
					<p className="text-3xl sm:text-4xl font-bold text-primary sm:mt-2 whitespace-nowrap">
						{estimatedHours} hs
					</p>
					<div>
				...
				<div className="mt-4 sm:mt-8 flex flex-col items-center gap-3 sm:gap-4">
					{ctaSent ? (
						...
						>
							<span className="material-icons text-2xl sm:text-3xl text-primary">
								check_circle
							</span>
							<p className="mt-1 sm:mt-2 text-base sm:text-lg font-semibold text-white">
								Nuestro equipo se va a comunicar con vos para coordinar una
								llamada.
							</p>
